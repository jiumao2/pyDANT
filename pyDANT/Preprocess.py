...
def preprocess(user_settings):
    ...
    Inputs:
        ...
        - channel_shanks.npy (optional): The shank ID of each channel. If missing,
    ...
    Outputs:
        ...
        - unit_shanks.npy: The shank ID of each unit, defined as the shank of its peak channel.
          If channel_shanks.npy is missing, this is a vector of ones for single-shank data.
    ...
    if os.path.isfile(channel_shanks_path):
        channel_shanks = np.load(channel_shanks_path).ravel()
        if len(channel_shanks) != channel_locations.shape[0]:
            raise ValueError('channel_shanks.npy should have one shank ID for each channel!')
    else:
        channel_shanks = np.ones(channel_locations.shape[0], dtype=np.int64)
...
def spikeInfo2npy(user_settings):
    ...
    np.save(os.path.join(data_folder, 'channel_shanks.npy'), Kcoords)
